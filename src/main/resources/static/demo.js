...
async function processAuthentication(formAsJson) {
	try {
		...
		var initResponse = await service.initAuth().chdForm(document.getElementById("card_details"), 'data-chd').send();
		...
		if ('AUTHENTICATION' == initResponse.status && ('NOT_REQUIRED' == initResponse.authenticationStatus || 'SUCCESSFUL' == initResponse.authenticationStatus)) {
			...
			var continueResponse = await service.continueAuth().setWorldlineSessionData(initResponse.worldlineSessionData).send();
...
async function processCardAfterAuthentication(deviceAPIRequest, worldlineSessionData) {
	...
	var response = await service.cardPayment().setWorldlineSessionData(worldlineSessionData).chdForm(document.getElementById("card_details"), 'data-chd').send();
...
async function processCard() {
	...
	try {
		displayResult("Processing Payment with Worldline.", "");
		var response = await service.cardPayment().chdForm(document.getElementById("card_details"), 'data-chd').setRequestTimeout(requestTimeout).send();
		displayResult("Processing result with merchant.", "");
		unpackResponse(response);
	}
	catch (err) {
...
async function initiateRedirect(formAsJson, pmMethodType) {
	try {
		displayResult("Processing Payment with Worldline.", "");
		if (pmMethodType == 'ibp') {
			var response = await service.redirectPayment().paymentForm(document.getElementById("online_banking_details"), 'data-ibp').send()
		}
		else if (pmMethodType == 'ewallet') {
			var response = await service.redirectPayment().paymentForm(document.getElementById("ewallet_details"), 'data-ewallet').send()
		}
		if (response.bankUrl) {
...
async function processEft(formAsJson) {
	try {
		displayResult("Processing Payment with Worldline.", "");
		var response = await service.eftPayment().paymentForm(document.getElementById("eft_details"), 'data-eft').send();
		displayResult("Processing result with merchant.", "");
		unpackResponse(response);
	}
	catch (err) {
