...
var year = new Date().getFullYear(),
  selectExp = document.getElementById('cardExpiryYear'),
  option = null,
  next_year = null;
...
async function exec(pmMethodType) {
  ...
  if (pmMethodType == 'card') {
    if (document.getElementById('authenticate').checked == true) {
      processAuthentication();
    } else {
      processCard(getCardDetails());
    }
  } else if (pmMethodType == 'ibp' || pmMethodType == 'ewallet') {
    initiateRedirect(pmMethodType);
...
function getResult(response) {
  ...
  result =
    appendIfNotBlank(
      result,
      response.eftPaymentSlipUrl,
      '<br>Payment Slip URL : ' + '<a href=' + response.eftPaymentSlipUrl + " target='_blank'>"
    ) + '</a>';
  return result;
}

function appendIfNotBlank(result, resValue, resField) {
  return (result += resValue ? resField + resValue : '');
}
...
function getRedirectIFrame(redirectFrame) {
  ...
  redirectIframe.style =
    'position:fixed; top:0px; left:0px; bottom:0px; right:0px; width:100%; height:100%; border:none; margin:0; padding:0; overflow:hidden; z-index:999999; background-color:#FFFFFF';
